...
import { STATIC_OPTIONS, WORKER_STATUS_OPTIONS, PAYOUT_TYPE_OPTIONS } from "@/lib/options";
...
export function WorkersView({ workers, options }: WorkersViewProps) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<WorkerWithSummary | null>(null);
  ...
  function openEdit(worker: WorkerWithSummary) { setEditTarget(worker); setForm(workerToForm(worker)); setFormOpen(true); }

  async function handleFormSubmit() {
    if (!form.worker_name.trim()) { toast.error("Worker name is required"); return; }
    setIsSubmitting(true);
    const body = { ...form, default_payout_rate: Number(form.default_payout_rate) || 0 };
    const result = editTarget
      ? await mutate(`/api/workers/${editTarget.worker_id}`, body)
      : await create("/api/workers", body);
    setIsSubmitting(false);
    if (result.ok) {
      toast.success(editTarget ? `${editTarget.worker_name} updated` : "Worker created");
      setFormOpen(false);
  ...
  const columns = getWorkerColumns({ onEdit: openEdit, onDelete: setDeleteTarget, isAdmin, isPending });
  ...
  return (
    <div className="mx-auto max-w-350 px-4 py-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <SearchInput value={search} onChange={setSearch} placeholder="Search name, phone…" className="w-55" />
        <FilterSelect value={statusFilter} onChange={setStatusFilter} options={WORKER_STATUS_OPTIONS} placeholder="All statuses" />
        ...
        {filtered.length !== workers.length && (
          <>
            <span className="text-xs text-muted-foreground">{filtered.length} of {workers.length} shown</span>
            <Button variant="ghost" size="sm" onClick={resetFilters} className="h-7 text-xs">Clear filters</Button>
          </>
        )}
      </div>

      <DataTable columns={columns} data={filtered} emptyMessage="No workers match your filters." />

      {/* Create / Edit Dialog */}
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editTarget ? `Edit ${editTarget.worker_name}` : "New Worker"}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label>Worker Name *</Label>
              <Input value={form.worker_name} onChange={(e) => setField("worker_name", e.target.value)} placeholder="e.g. Raju" />
            </div>
            <div className="space-y-1.5">
              <Label>Phone</Label>
              <Input value={form.phone} onChange={(e) => setField("phone", e.target.value)} placeholder="Mobile number" />
            </div>
            <div className="space-y-1.5">
              <Label>Primary Area</Label>
              <Select value={form.primary_area_id} onChange={(e) => setField("primary_area_id", e.target.value)}>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.default_payout_type} onChange={(e) => setField("default_payout_type", e.target.value)}>
                {PAYOUT_TYPE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Payout Rate (₹)</Label>
              <Input type="number" min="0" value={form.default_payout_rate} onChange={(e) => setField("default_payout_rate", e.target.value)} placeholder="0" />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleFormSubmit} disabled={isSubmitting}>
              {isSubmitting ? "Saving…" : editTarget ? "Save Changes" : "Create Worker"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove <strong>{deleteTarget?.worker_name}</strong>. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
