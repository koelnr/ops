...
export function getWorkerColumns(
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          className={`text-sm ${row.original.status === "Active" ? "text-green-700 dark:text-green-400" : "text-muted-foreground"}`}
        >
          {row.original.status || "—"}
        </span>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">
          {row.original.default_payout_type || "—"}
        </span>
      ),
    },
    {
