...
export function BookingsView({
  ...
  const [assignTarget, setAssignTarget] = useState<ResolvedBooking | null>(
    null,
  );
  ...
  const [deleteTarget, setDeleteTarget] = useState<ResolvedBooking | null>(
    null,
  );
  ...
  function openEdit(booking: ResolvedBooking) {
    setEditTarget(booking);
    setForm(resolvedBookingToForm(booking));
    setFormOpen(true);
  }
        ...
        {filtered.length !== resolvedBookings.length && (
          <>
            <span className="text-xs text-muted-foreground">
              {filtered.length} of {resolvedBookings.length} shown
            </span>
            <Button
              variant="ghost"
              size="sm"
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editTarget
                ? `Edit Booking ${editTarget.booking_id}`
                : "New Booking"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onChange={(e) => setField("time_slot_id", e.target.value)}
              >
                <option value="">— select slot —</option>
                {opts.timeSlots.map((o) => (
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={form.booking_status_id}
                onChange={(e) => setField("booking_status_id", e.target.value)}
              >
                <option value="">— select status —</option>
                {opts.bookingStatuses.map((o) => (
                  <option key={o.value} value={o.value}>
                    {o.label}
                  </option>
                ))}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                type="number"
                value={form.addon_total}
                onChange={(e) => setField("addon_total", e.target.value)}
                placeholder="0"
              />
            </div>
