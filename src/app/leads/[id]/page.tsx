...
export default async function LeadDetailPage({ params }: Props) {
  ...
  const serviceName = ctx?.services.get(lead.interested_service_id)?.name ?? lead.interested_service_id;
  const sourceLabel = ctx?.leadSources.get(lead.source_id)?.label ?? lead.source_id;
    ...
    <div className="mx-auto max-w-3xl px-4 py-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/leads"><ChevronLeft className="h-4 w-4" /></Link>
        </Button>
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">{lead.lead_id}</p>
        </div>
        {!isConverted && options && (
          <LeadConvertStepper
            leadId={lead.lead_id}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card>
          ...
          <CardContent className="space-y-1.5 text-sm">
            ...
            {areaName && <div className="text-muted-foreground">{areaName}</div>}
          </CardContent>
        </Card>
      ...
      {isConverted && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-green-700 dark:text-green-400">Converted</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            {convertedCustomer && (
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Customer</span>
                <Link href={`/customers/${convertedCustomer.customer_id}`} className="font-medium hover:underline">
            ...
            {convertedBooking && (
              <div className="flex justify-between items-center">
                ...
                <Link href={`/bookings/${convertedBooking.booking_id}`} className="font-mono text-xs hover:underline">
