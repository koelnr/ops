...
export default async function BookingDetailPage({ params }: Props) {
  ...
  const [bookings, bookingServices, payments, complaints, workers, vehicles, customers, ctx] =
    await Promise.all([
      getBookings().catch(() => []),
  ...
  if (!ctx) {
    return <div className="p-6 text-muted-foreground">Failed to load lookup data.</div>;
  }
