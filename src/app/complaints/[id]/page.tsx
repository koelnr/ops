...
export default async function ComplaintDetailPage({ params }: Props) {
  ...
  const customer = booking ? customers.find((c) => c.customer_id === booking.customer_id) : null;
  ...
  const workerOptions = workers.map((w) => ({ value: w.worker_id, label: w.worker_name }));
    ...
    <div className="mx-auto max-w-3xl px-4 py-6 space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/complaints"><ChevronLeft className="h-4 w-4" /></Link>
        </Button>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-semibold">{complaintType?.label ?? "Complaint"}</h1>
            <StatusBadge status={complaint.resolution_status} />
          </div>
          <p className="text-xs text-muted-foreground font-mono mt-0.5">{complaint.complaint_id}</p>
        </div>
        <ComplaintActions
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Issue</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm">
            ...
            {complaint.details && (
              <p className="text-muted-foreground pt-1 border-t">{complaint.details}</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            {booking && (
              <div className="flex justify-between items-center">
                ...
                <Link href={`/bookings/${booking.booking_id}`} className="font-mono text-xs hover:underline">
            ...
            {worker && (
              <div className="flex justify-between items-center">
                ...
                <Link href={`/workers/${worker.worker_id}`} className="hover:underline">
                  {worker.worker_name}
                </Link>
              </div>
            )}
