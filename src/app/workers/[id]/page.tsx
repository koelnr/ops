...
export default async function WorkerDetailPage({ params, searchParams }: Props) {
  ...
  const [workers, bookings, complaints, payments, customers, ctx] = await Promise.all([
    getWorkers().catch(() => []),
    getBookings().catch(() => []),
    getComplaints().catch(() => []),
    getPayments().catch(() => []),
  ...
  if (!worker) notFound();
  if (!ctx) return <div className="p-6 text-muted-foreground text-sm">Failed to load lookups.</div>;
  ...
  const workerComplaints = complaints.filter((c) => c.assigned_worker_id === id);

  const tabLinks = [
    { label: "Schedule", value: "schedule" as Tab, count: workerBookings.length },
    { label: "Complaints", value: "complaints" as Tab, count: workerComplaints.length },
  ];
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/workers"><ChevronLeft className="h-4 w-4" /></Link>
        </Button>
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground font-mono mt-0.5">{worker.worker_id}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent className="space-y-1.5 text-sm">
            {worker.phone && (
              <a href={`tel:${worker.phone}`} className="flex items-center gap-1 text-muted-foreground hover:text-foreground">
                <Phone className="h-3 w-3" />{worker.phone}
              </a>
            )}
            <div className="text-muted-foreground">{summary.areaName || "—"}</div>
            <div className="text-xs text-muted-foreground">Joined {worker.joining_date ? formatDate(worker.joining_date) : "—"}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Today</CardTitle></CardHeader>
          <CardContent>
            <div className="text-2xl font-semibold tabular-nums">{todayBookings.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-semibold tabular-nums">{summary.assignedCount}</div>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">Payout</CardTitle></CardHeader>
          <CardContent className="text-sm">
            <div className="font-medium">{worker.default_payout_type || "—"}</div>
            {worker.default_payout_rate > 0 && (
              <div className="text-muted-foreground">{formatCurrency(worker.default_payout_rate)}</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex gap-1 border-b">
        {tabLinks.map(({ label, value, count }) => (
          ...
          >
            {label}
            {count > 0 && <span className="text-xs rounded-full bg-muted px-1.5">{count}</span>}
          </Link>
        ))}
      </div>
                ...
                {workerBookings.slice(0, 50).map((b) => {
                  ...
                  return (
                    <TableRow key={b.booking_id}>
                      ...
                      <TableCell className="text-sm">{customer?.full_name ?? "—"}</TableCell>
                      <TableCell className="text-sm text-muted-foreground">{area?.name ?? "—"}</TableCell>
                      <TableCell><StatusBadge status={status?.label ?? b.booking_status_id} /></TableCell>
        ...
        workerComplaints.length === 0 ? (
          ...
          <div className="space-y-3">
            {workerComplaints.map((c) => {
              ...
              const customer = booking ? customers.find((cu) => cu.customer_id === booking.customer_id) : null;
              return (
