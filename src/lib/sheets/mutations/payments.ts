...
export async function updatePayment(id: string, patch: UpdatePaymentInput): Promise<void> {
  ...
  if (patch.amount_received !== undefined) cells.push(["D", String(patch.amount_received)]);
  if (patch.payment_mode_id !== undefined) cells.push(["E", patch.payment_mode_id]);
  if (patch.payment_status_id !== undefined) cells.push(["F", patch.payment_status_id]);
  if (patch.upi_transaction_ref !== undefined) cells.push(["G", patch.upi_transaction_ref]);
  if (patch.collected_by_worker_id !== undefined) cells.push(["H", patch.collected_by_worker_id]);
  if (patch.follow_up_required !== undefined) cells.push(["I", String(patch.follow_up_required)]);
