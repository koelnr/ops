...
export async function getAllVehicles(): Promise<Array<VehicleDoc & { id: string }>> {
  const snap = await vehiclesCol().get()
  return docsToArrayWithId(snap)
}

export async function getVehiclesByCustomerId(customerId: string): Promise<Array<VehicleDoc & { id: string }>> {
  const snap = await vehiclesCol().where('customerId', '==', customerId).get()
  return docsToArrayWithId(snap)
}

export async function createVehicleFromInput(input: CreateVehicleInput): Promise<string> {
...
export async function updateVehicleFromInput(id: string, patch: UpdateVehicleInput): Promise<void> {
  const updates: Record<string, unknown> = { updatedAt: nowTimestamp() }
  if (patch.registration_number !== undefined) updates.registrationNumber = patch.registration_number
  if (patch.car_model !== undefined) updates.carModel = patch.car_model
  if (patch.brand !== undefined) updates.brand = patch.brand
  if (patch.vehicle_type_id !== undefined) updates.vehicleTypeId = patch.vehicle_type_id
  if (patch.color !== undefined) updates.color = patch.color
  if (patch.parking_notes !== undefined) updates.parkingNotes = patch.parking_notes
  if (patch.is_primary_vehicle !== undefined) updates.isPrimaryVehicle = patch.is_primary_vehicle
