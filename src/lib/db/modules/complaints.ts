...
export async function getAllComplaints(): Promise<
  Array<ComplaintDoc & { id: string }>
> {
  const snap = await complaintsCol().get();
  return docsToArrayWithId(snap);
}

export async function getOpenComplaints(): Promise<
  Array<ComplaintDoc & { id: string }>
> {
...
export async function createComplaintFromInput(
  ...
  const [bookingSnap, typeSnap, workerSnap] = await Promise.all([
    ...
    input.complaint_type_id
      ? complaintTypesCol().doc(input.complaint_type_id).get()
      : null,
    input.assigned_worker_id
      ? workersCol().doc(input.assigned_worker_id).get()
      : null,
  ]);

  const booking = bookingSnap.data();
  const customer = booking
    ? await customersCol().doc(booking.customerId).get()
    : null;
  const customerData = customer?.data();
  const type = typeSnap?.data();
  const worker = workerSnap?.data();

  const resStatus = (input.resolution_status?.toLowerCase() ?? "open") as
    | "open"
    | "in_progress"
    | "resolved";
  ...
  const cmpData: Omit<ComplaintDoc, "createdAt" | "updatedAt"> = {
    ...
    complaintDate: input.complaint_date
      ? Timestamp.fromDate(new Date(input.complaint_date))
      : null,
...
export async function updateComplaintFromInput(
  id: string,
  ...
  if (patch.complaint_type_id !== undefined)
    updates.complaintTypeId = patch.complaint_type_id;
  if (patch.details !== undefined) updates.details = patch.details;
  if (patch.assigned_worker_id !== undefined)
    updates.workerId = patch.assigned_worker_id ?? null;
  if (patch.resolution_type !== undefined)
    updates.resolutionType = patch.resolution_type;
  if (patch.resolution_notes !== undefined)
    updates.resolutionNotes = patch.resolution_notes;
  if (patch.resolution_status !== undefined)
    updates.resolutionStatus = patch.resolution_status.toLowerCase();
  if (patch.follow_up_complete !== undefined)
    updates.followUpComplete = patch.follow_up_complete;
