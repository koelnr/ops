...
export async function getActiveCustomers(): Promise<Array<CustomerDoc & { id: string }>> {
  const snap = await customersCol().where('status', '==', 'active').get()
  return docsToArrayWithId(snap)
}

export async function getAllCustomers(): Promise<Array<CustomerDoc & { id: string }>> {
...
export async function createCustomerFromInput(input: CreateCustomerInput): Promise<string> {
...
export async function updateCustomerFromInput(id: string, patch: UpdateCustomerInput): Promise<void> {
  ...
  if (patch.secondary_phone !== undefined) updates.secondaryPhone = patch.secondary_phone ?? null
  if (patch.area_id !== undefined) updates.areaId = patch.area_id
  if (patch.full_address !== undefined) updates.fullAddress = patch.full_address
  if (patch.google_maps_link !== undefined) updates.googleMapsLink = patch.google_maps_link
  if (patch.landmark !== undefined) updates.landmark = patch.landmark
  if (patch.acquisition_source_id !== undefined) updates.acquisitionSourceId = patch.acquisition_source_id
