...
import { bookingServiceDoc, bookingServicesCol, servicesCol } from '../collections'
...
export async function getAllBookingServices(): Promise<Array<BookingServiceDoc & { id: string }>> {
...
export async function getBookingServicesByBookingId(
  ...
  const snap = await bookingServicesCol().where('bookingId', '==', bookingId).get()
  return docsToArrayWithId(snap)
}
