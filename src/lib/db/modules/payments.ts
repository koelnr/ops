...
import { bookingDoc, customerDoc, paymentDoc, paymentModesCol, paymentStatusesCol, paymentsCol, workersCol } from '../collections'
...
export async function getAllPayments(): Promise<Array<PaymentDoc & { id: string }>> {
  const snap = await paymentsCol().get()
  return docsToArrayWithId(snap)
}

export async function getPendingPayments(): Promise<Array<PaymentDoc & { id: string }>> {
  const snap = await paymentsCol().where('followUpRequired', '==', true).get()
  return docsToArrayWithId(snap)
}

export async function createPaymentFromInput(input: CreatePaymentInput): Promise<string> {
  ...
  const [bookingSnap, modeSnap, statusSnap, workerSnap] = await Promise.all([
    ...
    input.payment_mode_id ? paymentModesCol().doc(input.payment_mode_id).get() : null,
    input.payment_status_id ? paymentStatusesCol().doc(input.payment_status_id).get() : null,
    input.collected_by_worker_id ? workersCol().doc(input.collected_by_worker_id).get() : null,
  ])
  ...
  const payData: Omit<PaymentDoc, 'createdAt' | 'updatedAt'> = {
    ...
    paymentDate: input.payment_date ? Timestamp.fromDate(new Date(input.payment_date)) : null,
...
export async function updatePaymentFromInput(id: string, patch: UpdatePaymentInput): Promise<void> {
  ...
  if (patch.amount_received !== undefined) updates.amountReceived = patch.amount_received
  if (patch.payment_mode_id !== undefined) updates.paymentModeId = patch.payment_mode_id
  if (patch.payment_status_id !== undefined) updates.paymentStatusId = patch.payment_status_id
  if (patch.upi_transaction_ref !== undefined) updates.upiTransactionRef = patch.upi_transaction_ref ?? null
  if (patch.collected_by_worker_id !== undefined) updates.collectedByWorkerId = patch.collected_by_worker_id ?? null
  if (patch.follow_up_required !== undefined) updates.followUpRequired = patch.follow_up_required
