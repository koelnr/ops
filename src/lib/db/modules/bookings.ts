...
export async function getAllBookings(): Promise<Array<BookingDoc & { id: string }>> {
  const snap = await bookingsCol().get()
  return docsToArrayWithId(snap)
}

export async function getBookingsByDate(serviceDate: string): Promise<Array<BookingDoc & { id: string }>> {
  const snap = await bookingsCol().where('serviceDate', '==', serviceDate).get()
  return docsToArrayWithId(snap)
}

export async function createBookingFromInput(input: CreateBookingInput): Promise<string> {
  const now = nowTimestamp()

  // Look up display names for denormalized fields
  const [customerSnap, vehicleSnap, timeSlotSnap, statusSnap, workerSnap, areaSnap, sourceSnap] =
    await Promise.all([
      ...
      input.booking_status_id ? bookingStatusDoc(input.booking_status_id).get() : null,
  ...
  const data: Omit<BookingDoc, 'createdAt' | 'updatedAt'> = {
    ...
    vehicleLabel: vehicle ? `${vehicle.carModel}${vehicle.registrationNumber ? ` (${vehicle.registrationNumber})` : ''}` : '',
...
export async function updateBookingFromInput(id: string, patch: UpdateBookingInput): Promise<void> {
  ...
  if (patch.service_date !== undefined) updates.serviceDate = patch.service_date
  if (patch.time_slot_id !== undefined) updates.timeSlotId = patch.time_slot_id
  if (patch.booking_status_id !== undefined) updates.bookingStatusId = patch.booking_status_id
  if (patch.source_id !== undefined) updates.sourceId = patch.source_id
  if (patch.assigned_worker_id !== undefined) updates.assignedWorkerId = patch.assigned_worker_id
  if (patch.area_id !== undefined) updates.areaId = patch.area_id
  if (patch.base_price !== undefined) updates['pricing.basePrice'] = patch.base_price
  if (patch.discount_amount !== undefined) updates['pricing.discountAmount'] = patch.discount_amount
  if (patch.addon_total !== undefined) updates['pricing.addonTotal'] = patch.addon_total
