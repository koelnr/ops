...
export async function getAllWorkers(): Promise<Array<WorkerDoc & { id: string }>> {
  const snap = await workersCol().get()
  return docsToArrayWithId(snap)
}

export async function getActiveWorkers(): Promise<Array<WorkerDoc & { id: string }>> {
  const snap = await workersCol().where('status', '==', 'active').get()
  return docsToArrayWithId(snap)
}

export async function createWorkerFromInput(input: CreateWorkerInput): Promise<string> {
  ...
  const data: Omit<WorkerDoc, 'createdAt' | 'updatedAt'> = {
    ...
    primaryAreaName: '',
    joiningDate: input.joining_date ? Timestamp.fromDate(new Date(input.joining_date)) : null,
    status: (input.status?.toLowerCase() as 'active' | 'inactive') ?? 'active',
    payout: {
      type: (input.default_payout_type as 'per_job' | 'daily' | 'monthly') ?? 'per_job',
...
export async function updateWorkerFromInput(id: string, patch: UpdateWorkerInput): Promise<void> {
  ...
  if (patch.primary_area_id !== undefined) updates.primaryAreaId = patch.primary_area_id
  ...
  if (patch.default_payout_type !== undefined) updates['payout.type'] = patch.default_payout_type
  if (patch.default_payout_rate !== undefined) updates['payout.rate'] = patch.default_payout_rate
